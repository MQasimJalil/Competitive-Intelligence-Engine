            ...
            try:
                ...
                async def guard_request(route, request) -> None:
                    if request.resource_type in {"image", "media", "font"}:
                        await route.abort()
                        return
                    if not rendered_request_is_allowed(
                        request.url,
                        source_url,
                ...
                if not (
                    is_same_site_url(final_url, source_url)
                    or is_same_site_url(source_url, final_url)
                ):
                    raise FetchError(
...
def rendered_request_is_allowed(
    request_url: str,
    source_url: str,
    *,
    resource_type: str,
    ...
    if resource_type in {"image", "media", "font"}:
        return False
    try:
        safe_url = validate_public_url(request_url)
        assert_resolves_to_public_ips(normalize_domain(safe_url))
    except ValueError:
        return False
    if not is_navigation:
        return True
    return is_same_site_url(safe_url, source_url) or is_same_site_url(source_url, safe_url)
